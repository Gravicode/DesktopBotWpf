...
public partial class ChatHistoryViewModel : ObservableObject
{
    ...
    [RelayCommand]
    public void Backup() => OrganizeDesktopPlugin.Backup();
    ...
    public async Task Submit()
    {
        ...
        ProcessingBotResponse = true;
        var response = await _manager.GenerateResponse(prompt ?? "");
        ProcessingBotResponse = false;
        Messages.Add(new Message() { Content = $"Bot: ", Role = "Assistant", Text = response });
    }

    public Message GetLastBotMessage() => Messages.Last(m => m.Role == "Assistant");
}
